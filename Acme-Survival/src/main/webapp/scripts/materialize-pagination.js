 ...
 * 
 * Sample usage: $('#pagination').materializePagination({ align: 'left', lastPage: 10, firstPage: 1, urlParameter: 'page', useUrlParameter: true, onClickCallback:
 * function(requestedPage){ console.log('Requested page is '+ requestedPage) } });
 */
;
(function($, window, document, undefined) {
	...
	MaterializePagination.prototype = {
		...
		createPaginationBase : function(requestedPage) {
			...
			this.$container = $('<ul>').addClass('pagination').addClass(this.config.align + '-align');
		...
		renderEllipsis : function() {
			if (this.$prevEllipsis.isHidden && this.currentPage >= this.config.firstPage + this.maxVisiblePages)
				this.$prevEllipsis.show();

			else if (!this.$prevEllipsis.isHidden && this.currentPage <= this.config.firstPage + this.maxVisiblePages - 1)
				this.$prevEllipsis.hide();

			if (this.$nextEllipsis.isHidden && this.currentPage <= this.config.lastPage - this.maxVisiblePages)
				this.$nextEllipsis.show();

			else if (!this.$nextEllipsis.isHidden && this.currentPage >= this.config.lastPage - this.maxVisiblePages + 1)
		...
		updateUrlParam : function(key, value) {
			...
			].join(''), urlQueryString = document.location.search, newParam = key + '=' + value, params = '?' + newParam;
		...
		util : {
			createPage : function(pageData) {
				return $('<li>').html('<a>' + pageData + '</a>').addClass('waves-effect').attr('data-page', pageData);
			},
			createChevron : function(type) {
				var direction = type === 'next' ? 'right' : 'left';

				var $icon = $('<i>').addClass('waves-effect').addClass('material-icons').text('chevron_' + direction);
