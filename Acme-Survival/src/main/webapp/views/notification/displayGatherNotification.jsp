...
<jstl:if test="${attackId != null}">
	<acme:button url="attack/player/delete.do?attackId=${attackId}"
		code="notification.finish.result" />
</jstl:if>

<jstl:if test="${gatherId != null}">
<%-- 	<acme:button code="mission.summary" --%>
<%-- 		url="gather/player/foundItems.do?notificationId=${notification.id}" /> --%>
		<acme:button code="mission.summary" url="/" /> 
</jstl:if>
...
<jstl:if test="${notification.gather!=null}">
	...
	<a class="waves-effect waves-light btn modal-trigger"
		href="#modalEvents"><spring:message
			code="notification.modal.events" /></a>
	<!-- Modal Structure -->
	<div id="modalEvents" class="modal">
		<div class="modal-content">
			<h4>
				<spring:message code="notification.modal.title" />
			</h4>
			<jstl:if test="${fn:length(notification.events) gt 0}">
				<p>
					<spring:message code="notification.modal.description" />
				</p>
				<jstl:forEach items="${notification.events}" var="event">
					...
					<br />
					<spring:message code="notification.modal.event.health" />
					<jstl:out value="${event.health}" />, <spring:message
						code="notification.modal.event.food" />
					<jstl:out value="${event.food}" />
					<spring:message code="notification.modal.event.water" />
					<jstl:out value="${event.water}" />
					<br />
		...
		<div class="modal-footer">
			<a
				href="notification/player/displayGatherNotification.do?notificationId=${notification.id}"
				class="modal-close waves-effect waves-green btn-flat"><spring:message
					code="notification.modal.accept" /></a>
		</div>
	</div>
</jstl:if>